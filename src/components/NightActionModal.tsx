...
interface NightActionModalProps {
  ...
  ghostEliminatedPlayerId: number | null;
  setGhostEliminatedPlayerId: (id: number | null) => void;
          ...
          {currentNightStep < filteredSteps.length - 1 ? (
            ...
            <button 
              onClick={() => {
                ...
                // Ghost Death Logic (Night 4)
                if (nightNumber === 4) {
                  const ghostAgent = players.find(p => p.role === 'Agent Fantôme' && p.status === 'active');
                  const target = players.find(p => p.id === ghostTargetId);
                  // Check if target is being killed by spies right now
                  const targetKilledBySpies = spyTargetId && doctorSavedId !== spyTargetId && spyTargetId === ghostTargetId;
                  
                  if (ghostAgent && target && target.status === 'active' && !ghostSuccess && !targetKilledBySpies) {
                    setGhostEliminatedPlayerId(ghostAgent.id);
                  } else {
